...
def generate_content(topic: str):
    ...
    try:
        ...
        title = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user", 
                "content": f"Создайте привлекательный и точный заголовок для статьи на тему '{topic}', с учётом актуальных новостей:\n{recent_news}. Заголовок должен быть интересным и актуальным."
            }],
            max_tokens=50,
            temperature=0.7,
            stop=["\n"]
        ...
        meta_description = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user", 
                "content": f"Напишите мета-описание для статьи с заголовком '{title}', которое должно быть информативным, полным, и четко отражать суть статьи, основываясь на последних новостях:\n{recent_news}."
            }],
            max_tokens=150,
            temperature=0.7,
            stop=["."]
        ...
        post_content = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user", 
                "content": f"Напишите подробный и глубокий пост на тему '{topic}', используя последние новости:\n{recent_news}. Пост должен быть логичным, связным, содержательным и завершённым. Объем не менее 1500 символов. Поделитесь актуальной информацией, деталями, аналитикой по теме."
            }],
            max_tokens=1500,  # Увеличено количество токенов для генерации длинного контента
            temperature=0.7,
            stop=["\n"]
        ).choices[0].message.content.strip()
